...
sudo env LC_ALL=C.UTF-8 timeshift --create
...
if [[ $SNAPSHOT_COUNT -ge $SNAPSHOT_COUNT_LIMIT ]]; then
	...
	sudo env LC_ALL=C.UTF-8 timeshift --delete --snapshot $(sudo timeshift --list | grep "0    >" | grep -Eo "[0-9]{4}-[0-9]{2}-[0-9]{2}_[0-9]{2}-[0-9]{2}-[0-9]{2}")
else
	echo -e "${BOLD}Less than ${SNAPSHOT_COUNT_LIMIT} snapshots present, no cleanup${NC}"
fi

echo -e "${BOLD}Present snapshots${NC}"
sudo env LC_ALL=C.UTF-8 timeshift --list
